    ...
    <div class="grid grid-cols-1 gap-4 mx-4">
        <div class="bg-white w-full rounded-lg transition-all duration-300 ease-in-out" data-aos="fade-up"
            ...
            <div class="flex justify-between">
                <div class="min-h-full w-full">
                    <div class="flex lg:items-center">
                        <div class="bg-[#F3F3F3] flex w-[10%] pb-2 lg:pb-4">
                            <div class="bg-gradient-to-b from-amber-500 to-yellow-300 shadow-xl w-[90%] flex items-center justify-center p-5 rounded-s-lg"
                        ...
                        <div class="flex justify-between w-full items-center pt-5 lg:pt-0">
                            <div class="flex flex-col">
                                <p class="text-3xl font-semibold text-slate-600 px-12">{{$history->nama_user}}</p>
                                <p class="text-lg px-12 mt-1">{{$history->nim}}</p>
                            </div>
                            <div class="mx-12 bg-gradient-to-b from-teal-500 to-green-400 shadow-xl rounded-full">
                                <p class="text-white p-5 text-3xl font-semibold">{{App\Helpers\GlobalFunction::pembulatan($history->rata)}}</p>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="border-l-4 border-yellow-300 pl-6 mt-6">
                            <div class="overflow-x-auto">
                                <table class="table text-slate-700">
                                    ...
                                    <tbody>
                                        @foreach ($history->hasilQuiz as $quiz)
                                            <tr>
                                                <th>{{$loop->iteration??''}}</th>
                                                <td>{{$quiz->nama_materi??''}}</td>
                                                <td>{{$quiz->nilai??''}}</td>
                                            </tr>
                ...
                <div class="bg-gradient-to-b from-amber-500 to-yellow-300 shadow-xl min-h-full min-w-6 border rounded-e-lg">
                </div>
            </div>
        </div>
    </div>
